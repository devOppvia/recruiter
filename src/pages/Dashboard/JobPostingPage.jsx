...
const JobPostingPage = () => {
  ...
  const [jobToMoveToReview, setJobToMoveToReview] = useState(null);
  ...
  const moveToReview = async () => {
    try {
      ...
      const response = await updateJobStatusApi(jobToMoveToReview, {
        jobStatus: "REVIEW",
  ...
  const fetchPurchasedPlans = async (jobId) => {
    setIsLoadingPlans(true);
    setJobToMoveToReview(jobId);
    try {
