...
import { X, MapPin, Clock, Users, Briefcase, IndianRupee, Star, ShieldCheck, Share2 } from 'lucide-react';
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
                    ...
                    >
                        ...
                        <div className="w-full md:w-[350px] bg-brand-primary/5 p-8 flex flex-col border-r border-brand-primary/5">
                            <div className="mb-8">
                                <Badge status={job.status} size="sm" className="mb-6" />
                            ...
                            <div className="space-y-6 flex-1">
                                <div className="space-y-4">
                                    {[
                                        { icon: <MapPin size={16} />, label: 'Work Type', value: job.workType },
                                        { icon: <Clock size={16} />, label: job.applicationType === 'Job' ? 'Notice Period' : 'Duration', value: job.duration },
                                        { icon: <Users size={16} />, label: job.applicationType === 'Job' ? 'Openings' : 'Interns Required', value: job.internsRequired },
                                        { icon: <Briefcase size={16} />, label: 'Hours', value: job.workingHours },
                                    ].map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-4 group">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-bold text-brand-primary/30 uppercase tracking-widest">{item.label}</p>
                                                <p className="text-sm font-black text-brand-primary">{item.value}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="pt-6 border-t border-brand-primary/10">
                                    <p className="text-[10px] font-bold text-brand-primary/30 uppercase tracking-widest mb-3">Compensation</p>
                                    <div className="p-4 bg-white rounded-2xl shadow-soft">
                                        <div className="flex items-center gap-3 text-brand-primary mb-1">
                                            ...
                                            <span className="text-lg font-black">
                                                {job.applicationType === 'Job'
                                                    ? `₹${job.salary?.minAmount} - ${job.salary?.maxAmount} LPA`
                                        ...
                                        <p className="text-[10px] font-black text-brand-primary/40 uppercase tracking-widest ml-7">
                                            {job.applicationType === 'Job' ? 'Annual (CTC)' : 'Monthly Stipend'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="max-w-2xl space-y-12">
                                ...
                                <section className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-black text-brand-primary uppercase tracking-widest">Role Description</h3>
                                    </div>
                                ...
                                <section className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-black text-brand-primary uppercase tracking-widest">Required Skills</h3>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {job.skills.map((skill) => (
                                            <span key={skill} className="px-4 py-2 bg-brand-primary/5 text-brand-primary text-xs font-black uppercase tracking-wide rounded-xl border border-brand-primary/5">
                                                {skill}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {job.benefits?.length > 0 && (
                                    <section className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <h3 className="text-sm font-black text-brand-primary uppercase tracking-widest">Perks & Benefits</h3>
                                        </div>
                                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            {job.benefits.map((benefit, idx) => (
                                                <li key={idx} className="flex items-start gap-2 text-sm font-bold text-brand-primary/60">
                                ...
                                {job.otherInfo && (
                                    <section className="p-8 bg-brand-primary/5 rounded-[32px] border border-brand-primary/5 space-y-4 mb-10">
                                        <h3 className="text-sm font-black text-brand-primary uppercase tracking-widest">Additional Information</h3>
