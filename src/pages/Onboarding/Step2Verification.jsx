...
import {
  verifyCompanyMobileNumOtpApi,
  resendCompanyMobileNumOtpApi,
  verifyCompanyEmailOtpApi,
  resendCompanyEmailOtpApi,
} from "../../helper/api";
  ...
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="text-[13px] font-black text-brand-primary uppercase tracking-widest">
            {label}
          </h4>
          <p className="text-[11px] font-bold text-brand-primary/40 truncate max-w-[200px]">
            {target}
          </p>
        </div>
      </div>
      <div className="flex items-center self-end sm:self-auto shrink-0">
        {canResend ? (
          <button
            onClick={onResend}
            disabled={disabled}
            className="text-[10px] font-black text-brand-primary hover:text-brand-primary-light flex items-center gap-1.5 uppercase tracking-widest bg-brand-primary/5 px-4 py-2 rounded-xl border border-brand-primary/10 transition-all disabled:opacity-50"
          >
    ...
    <div className="flex gap-3 justify-evenly">
      {value.map((digit, idx) => (
        <input
          ...
          className="w-full aspect-square max-w-[62px] text-center text-2xl font-black text-brand-primary bg-brand-primary/5 border border-brand-primary/10 rounded-2xl focus:bg-white focus:border-brand-primary/30 focus:ring-4 focus:ring-brand-primary/5 outline-none transition-all duration-300 disabled:opacity-50"
        />
      ))}
    </div>
  </div>
...
const Step2Verification = ({ onNext, onBack, data }) => {
  const [timers, setTimers] = useState({
    email: { timeLeft: 10, canResend: false },
    phone: { timeLeft: 10, canResend: false },
  });
  ...
  const handleResend = async (type) => {
    try {
      ...
      const payload =
        type === "email" ? { email: data.email } : { mobileNumber: data.phone };

      const response = await apiCall(companyId, payload);

      if (response.status) {
        toast.success(response.message || `OTP resent to your ${type}`);
      ...
      toast.error(error?.message || `Failed to resend ${type} OTP`);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (emailOtpStr.length < 4 || phoneOtpStr.length < 4) {
      toast.error("Please enter both 4-digit OTP codes");
      return;
    }
    ...
    try {
      ...
      const emailResponse = await verifyCompanyEmailOtpApi(companyId, {
        otp: emailOtpStr,
      });

      if (!emailResponse.status) {
        toast.error(`Email Error: ${emailResponse.message || "Invalid OTP"}`);
        setIsLoading(false);
        return;
      }

      // Step 2: Verify Mobile OTP
      const mobileResponse = await verifyCompanyMobileNumOtpApi(companyId, {
        otp: phoneOtpStr,
      });
      ...
      if (mobileResponse.status) {
        ...
        toast.error(`Mobile Error: ${mobileResponse.message || "Invalid OTP"}`);
      }
    } catch (error) {
      console.log("Error:", error);
    ...
    <div className="space-y-12">
      ...
      <div className="flex items-start gap-4 p-6 bg-brand-primary/2 border border-brand-primary/5 rounded-[32px] relative overflow-hidden">
        ...
        <div className="space-y-1">
          <h5 className="text-[10px] font-black text-brand-primary uppercase tracking-widest">
            Protocol Intelligence
          </h5>
          <p className="text-[11px] font-bold text-brand-primary/40 leading-relaxed max-w-sm">
            Verification tokens expire in 10 cycles. Multi-layer encryption
            ensures your communication remains strictly confidential within the
            Oppvia ecosystem.
          </p>
        </div>
      </div>
